      ...
      /* 상품페이지 */
          try{
            ...
            let select1 = await page.evaluate(async ()=>{
              return $('.sstpl_opt.pt15').length;
            })
            let select2 = await page.evaluate(async ()=>{
              return $('.product_info_detail .product_info_detailDL').length;
            })
            //page.$$(".sstpl_opt.pt15");
            if(select1 > 0 ){
               /* 셀렉트박스 1 */
               ...
               beforePos = await page.evaluate(async ()=>{
                ...
                await page.click(".sstpl_opt_selWrap .selbox:last-child > .iList li:last-child");
                for(i=0;i<10;i++){
